...
/** Raw HTML for the map popup visit CTA (outside React). */
export function visitProviderButtonMarkup(
  ...
  if (!url) return "";
  return `<a href="${esc(url)}" target="_blank" rel="noopener noreferrer" style="display:flex;align-items:center;justify-content:center;gap:6px;margin-top:12px;padding:9px 14px;border-radius:8px;background:#1E3A4C;color:#ffffff;font-family:var(--font-body),system-ui,sans-serif;font-size:12px;font-weight:600;text-decoration:none;width:100%;box-sizing:border-box;transition:background 0.15s;" onmouseover="this.style.background='#14344A'" onmouseout="this.style.background='#1E3A4C'">
    ${esc(label)}
